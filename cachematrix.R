## Given a invertible matrix, the following two functions will calculate the 
## inverse matrix or retrieve the inverse matrix from the cache.
...
## (4)They don’t calculate the inverse, they simply store the value of the input 
## in a variable m.
## (5)into the main function makeCacheMatrix (setinverse) and return it 
## (getinverse).
...
## the input of cacheSolve) returned by makeCacheMatrix above. If the inverse has
...
## calculated, data gets the matrix stored with makeCacheMatrix, m calculates the 
## inverse, and x$setinverse(m) stores it in the object m in makeCacheMatrix.
